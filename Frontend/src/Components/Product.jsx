...
function Product({ product, enableQuickAdd = false }) {
  ...
  const handleQuickAdd = (e) => {
    ...
    // If product has variants, pick first in-stock; otherwise use legacy product price
    const variants = Array.isArray(product?.variants) ? product.variants : [];
    let variantToAdd = null;
    if (variants.length) {
      variantToAdd = variants.find((v) => (v?.stock || 0) > 0) || variants[0];
    } else {
      // Create a pseudo-variant from product
  ...
  const impRef = useImpression('product_impression', { id: product?._id });
  const { user } = { user: null }; // shallow guard if auth not loaded
  const { has, toggle } = useWishlist();
  const wished = has(product?._id || product?.id);

  return (
    <Card ref={impRef} className="my-3 p-3 rounded product-card" style={{ borderRadius: 16 }}>
      <Link to={`/product/${product._id}`} onClick={(e) => { if (enableQuickAdd) e.preventDefault(); track('product_click', { id: product?._id }); }}>
        <div style={{ position: 'relative', width: '100%', paddingTop: '125%' /* 4:5 aspect ratio */ }}>
          ...
          <div className="d-none d-md-block position-absolute" style={{ right: 8, bottom: 8 }}>
            {enableQuickAdd && (
              <button
                ...
                onClick={(e)=>{ handleQuickAdd(e); track('quick_add', { id: product?._id }); }}
              >
                Quick Add
              </button>
            )}
          ...
          <button
            ...
            onClick={(e)=>{ e.preventDefault(); toggle(product?._id || product?.id); }}
          >
      ...
      <Card.Body>
        <Link to={`/product/${product._id}`} className="product-link" onClick={(e) => enableQuickAdd && e.preventDefault()}>
          <Card.Title as="div">
